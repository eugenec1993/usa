...
under your price limit, and emails you. A state file stops repeat alerts while
still allowing a new alert when the same ticket grouping drops in price.
...
def load_state():
    ...
    # Migrate the old {"alerted": [...]} state format automatically.
...
def main():
    ...
    state = load_state()
    best_prices = state.setdefault("best_prices", {})
    fresh = []

    for m in all_matches:
        # A stable grouping key lets the same listing group alert again only
        # when its price becomes lower than the last price that triggered.
        key = f"{m['source']}|{m['section']}|{m['qty']}"
        ...
        if previous is None or m["price"] < previous:
    ...
    for name, items in by_src.items():
        rows = []
        for m in items:
            if m["previous_price"] is None:
                change = "new qualifying listing"
            else:
                change = f"down from ${m['previous_price']:.0f}"
            rows.append(
                f"  Section {m['section']} — {m['qty']} seats — "
                f"${m['price']:.0f} each ({change})"
            )
    ...
    send_email(
        f"MATCH: {len(fresh)} new or cheaper lower-level set(s) under ${PRICE_LIMIT:.0f}",
        body,
    )

    # Only update state after the alert is successfully sent/printed.
    for m in fresh:
        key = f"{m['source']}|{m['section']}|{m['qty']}"
        best_prices[key] = m["price"]

    # Remove obsolete legacy state after successful migration.
